...
router.post('/', [
    ...
    body('price').isInt({min: 1}).withMessage('has to be a number and greater than 1'),
    body('images').isObject().withMessage('has to be an object'),
    body('stock_status').isString().withMessage('has to be a string'),
    body('stock_quantity').isInt({min: 0}).withMessage('has to be an number and greater than 1'),
